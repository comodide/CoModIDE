...
/**
 * Singleton class for accessing a pre-indexed OPLa-compliant ontology pattern
 * library.
 * 
...
public class PatternLibrary
{
	...
	public OWLOntology getOwlRepresentation(Pattern pattern) throws OWLOntologyCreationException
	{
		...
		if (is == null)
		{
			throw new OWLOntologyCreationException(
					String.format("The OWL representation path '%s' could not be read as an input stream.",
							pattern.getOwlRepresentationPath()));
		}
		return exportManager.loadOntologyFromOntologyDocument(is);
	}

	/**
	 * Parses a pattern index and updates the data structures that are needed to
	 * feed other classes.
	 */
			...
			for (OWLIndividual pattern : EntitySearcher.getIndividuals(patternClass, index))
			{
				if (pattern.isNamed())
				{
					...
					if (patternLabels.size() > 0 && owlRepresentations.size() > 0)
					{
						...
						// Find all the categories for this pattern. All patterns are assigned to at
						// least the Any category by default
						List<PatternCategory> categoriesForPattern = new ArrayList<PatternCategory>();
						categoriesForPattern.add(ANY_CATEGORY);
						for (OWLIndividual category : EntitySearcher.getObjectPropertyValues(namedPattern,
								categorizationProperty, index))
						{
							if (category.isNamed())
							{
								...
								PatternCategory newCategory = new PatternCategory(categoryLabel,
										namedCategory.getIRI());
	...
	/**
	 * Convenience method that returns the string values of all rdfs:label
	 * annotations on a given entity in a given ontology.
	 * 
	 * @param entity
	 * @param ontology
	 * @return
	...
	/**
	 * Convenience method that returns the string values of all occurrences of a
	 * given data property on a given individual in a given ontology
	 * 
	 * @param i        - Individual
	 * @param p        - Data property
	 * @param ontology - Ontology
	 * @return List of strings, one per each occurrence of the data property, on the
	...
	/**
	 * Returns a list of all categories (that have one or more patterns) in this
	 * library.
	 * 
	 * @return
	 */
