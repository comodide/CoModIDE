...
public class BFOView extends AbstractOWLViewComponent implements ComodideMessageHandler
{
	...
	public void initialiseOWLView() throws Exception
	{
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, cellPanel, edgePanel);
		...
		independentContinuant_Checkbox.addItemListener(new ItemListener()
		{
			...
			public void itemStateChanged(ItemEvent arg0)
			{
				...
				if (checked)
				{
					...
					/*
					 * if (owlOntology != null) { owlDataFactory =
					 * owlOntology.getOWLOntologyManager().getOWLDataFactory();
					 * OWLObjectSomeValuesFrom someValuesFrom =
					 * owlDataFactory.getOWLObjectSomeValuesFrom(property, source.asOWLClass());
					 * log.info("addAxiomChange"+ someValuesFrom); OWLSubClassOfAxiom subClassAxiom
					 * = owlDataFactory.getOWLSubClassOfAxiom(someValuesFrom, target.asOWLClass());
					 * log.info("addAxiomChange"+ subClassAxiom); AddAxiom addAxiomChange = new
					 * AddAxiom(owlOntology, subClassAxiom); log.info("addAxiomChange"+
					 * addAxiomChange); modelManager.applyChange(addAxiomChange); }
