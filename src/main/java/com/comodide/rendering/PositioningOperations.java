...
//	private static OWLAnnotationProperty entityPositionY = createOplasdProperty("entityPositionY");
	 ...
	 *         are found in the annotations, each of the coordinates returned are
	 *         randomized doubles between 0 and 300 (to scatter the rendered nodes a
	 *         little and avoid 100 % overlap)
	 */
	public static Pair<Double, Double> getXYCoordsForEntity(OWLEntity entity, OWLOntology ontology)
	{
		log.debug(String.format("Parsing OPLa-SD annotations to establish coordinates for entity '%s'",
				entity.toString()));

		// Default values if no annotations are found; randomized to scatter the nodes a
		// little so they don't all overlap exactly
		Double positionX = getRandomDoubleBetweenRange(0.0, 300.0);
		Double positionY = getRandomDoubleBetweenRange(0.0, 300.0);
		log.debug(String.format("Default (auto-generated) coordinates for entity '%s': (%s,%s)", entity.toString(),
				positionX.toString(), positionY.toString()));

		// We'll iterate through all the annotations, so in effect we will get one at
	...
	public static void updateXYCoordinateAnnotations(OWLEntity entity, OWLOntology ontology, Double newX, Double newY)
	{
		...
		for (OWLAnnotation positionAnnotation : positionAnnotations)
		{
			if (positionAnnotation.anonymousIndividualValue().isPresent())
			{
				OWLAnonymousIndividual           positionIndividual     = positionAnnotation.anonymousIndividualValue()
						.get();
				Set<OWLAnnotationAssertionAxiom> positionSubAnnotations = ontology
						.getAnnotationAssertionAxioms(positionIndividual);
				removedAxioms.addAll(positionSubAnnotations);
				removedAxioms.add(
						factory.getOWLAnnotationAssertionAxiom(entityPosition, entity.getIRI(), positionIndividual));
			}
		}
		...
		OWLLiteral                  comment                                  = factory.getOWLLiteral(
				"This is an entity positioning annotation generated by CoModIDE (https://comodide.com/). Removing this annotation will break rendering the CoModIDE schema diagram view.",
				"en");
		OWLAnnotationAssertionAxiom positionXAnnotationAxiom                 = factory
				.getOWLAnnotationAssertionAxiom(entityPositionX, positionAnnotationIndividual, positionX);
		OWLAnnotationAssertionAxiom positionYAnnotationAxiom                 = factory
				.getOWLAnnotationAssertionAxiom(entityPositionY, positionAnnotationIndividual, positionY);
		OWLAnnotationAssertionAxiom positionAnnotationIndividualCommentAxiom = factory
				.getOWLAnnotationAssertionAxiom(factory.getRDFSComment(), positionAnnotationIndividual, comment);
		...
		OWLAnnotationAssertionAxiom entityAnnotationAxiom = factory.getOWLAnnotationAssertionAxiom(entityPosition,
				entity.getIRI(), positionAnnotationIndividual);
