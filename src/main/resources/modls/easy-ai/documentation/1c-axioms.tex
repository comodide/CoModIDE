...
\end{center}
\caption{Schema Diagram for the 1c-EASY-AI pattern. %The visual notation is explained in Chapter \ref{chap:prelims}.
}
...
\begin{align}
% General 2, 5, 8, 12, 16
\textsf{Actor} \sqcap \textsf{InputRole} &\sqsubseteq~\bot \\ %2
% Domain and Range Restrictions
\textsf{$\top$} &\sqsubseteq~\forall\textsf{performsInputRole.InputRole} \\ %5 
% Existential
\textsf{InputRole} &\sqsubseteq~\exists\textsf{performsInputRole}^-\textsf{.Actor} \\ %8
% Functionality
\textsf{Actor} &\sqsubseteq~\mathord{\geq}1\textsf{performsInputRole.InputRole} \\ %12
% Inverses
\textsf{InputRole} &\sqsubseteq~\mathord{\geq}1\textsf{performsInputRole}^-\textsf{.Actor} %16
\end{align}
%%%%%
%%%%%
...
% Actor-pIR-IR 2, 5, 8, 12, 16
    ...
    \item  Qualified Scoped Functionality: For every \textsf{Actor}, there is at most one R-filler in \textsf{InputRole} % Every \textsf{InputRole} has at most one \textsf{performsInputRole} that points to it.
    \item  Inverse Qualified Scoped Functionality: for every \textsf{InputRole}, there is at most one inverse R-filler in \textsf{Actor} % Every \textsf{InputRole}, \txtsf{perfomsInputRole} relates to it. 
