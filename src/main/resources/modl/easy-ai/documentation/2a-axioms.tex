...
\end{center}
\caption{Schema Diagram for the 2a-EASY-AI pattern. %The visual notation is explained in Chapter \ref{chap:prelims}.
}
...
\begin{align}
% General 2, 5, 8, 12, 16
\textsf{(Artifact or Model)} \sqcap \textsf{InputRole} &\sqsubseteq~\bot \\ %2
% Domain and Range Restrictions
\textsf{$\top$} &\sqsubseteq~\forall\textsf{performsInputRole.InputRole} \\ %5 
% Existential
\textsf{InputRole} &\sqsubseteq~\exists\textsf{performsInputRole}^-\textsf{.(Artifact or Model)} \\ %8
% Functionality
\textsf{(Artifact or Model)} &\sqsubseteq~\mathord{\geq}1\textsf{performsInputRole.InputRole} \\ %12
% Inverses
\textsf{InputRole} &\sqsubseteq~\mathord{\geq}1\textsf{performsInputRole}^-\textsf{.(Artifact or Model)} %16
\end{align}
%%%%%
%%%%%
...
\begin{align}
    % General 2, 5, 8, 12, 16
    \textsf{Artifact} \sqcap \textsf{OutputRole} &\sqsubseteq~\bot \\ %2
    % Domain and Range Restrictions
    \textsf{$\top$} &\sqsubseteq~\forall\textsf{performsOutputRole.OutputRole} \\ %5 
    % Existential
    \textsf{OutputRole} &\sqsubseteq~\exists\textsf{performsOutputRole}^-\textsf{.Artifact} \\ %8
    % Functionality
    \textsf{Artifact} &\sqsubseteq~\mathord{\geq}1\textsf{performsOutputRole.OutputRole} \\ %12
    % Inverses
    \textsf{OutputRole} &\sqsubseteq~\mathord{\geq}1\textsf{performsOutputRole}^-\textsf{.Artifact} %16
\end{align}
%%%%%
%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{enumerate}
% (Artifact or Model)-pIR-IR 2, 5, 8, 12, 16
    ...
    \item  Qualified Scoped Functionality: For every \textsf{(Artifact or Model)}, there is at most one R-filler in \textsf{InputRole} % Every \textsf{InputRole} has at most one \textsf{performsInputRole} that points to it.
    \item  Inverse Qualified Scoped Functionality: for every \textsf{InputRole}, there is at most one inverse R-filler in \textsf{(Artifact or Model)} % Every \textsf{InputRole}, \txtsf{perfomsInputRole} relates to it. 
% Artifact-pOR-OR 2, 5, 8, 12, 16
    ...
    \item  Qualified Scoped Functionality: For every \textsf{Artifact}, there is at most one R-filler in \textsf{OutputRole} % Every \textsf{InputRole} has at most one \textsf{performsInputRole} that points to it.
    \item  Inverse Qualified Scoped Functionality: for every \textsf{OutputRole}, there is at most one inverse R-filler in \textsf{Artifact} % Every \textsf{InputRole}, \txtsf{perfomsInputRole} relates to it. 
